...
class ContourAnalyzer(object):
    ...
    def compute_missing_cell_contours(self, missing_cells_mask):
        contx, _ = cv2.findContours(missing_cells_mask, cv2.RETR_LIST, cv2.CHAIN_APPROX_TC89_KCOS)
        return contx
    ...
    def normalize(self, arr):
        ...
        y2 = np.max(arr[:, 1])
        for i in arr:
            if(i[0]+1 == x1 or i[0]-1 == x1):
                i[0] = x1
            if(i[0]+1 == x2 or i[0]-1 == x2):
                i[0] = x2
            if(i[1]+1 == y1 or i[1]-1 == y1):
                i[1] = y1
            if(i[1]+1 == y2 or i[1]-1 == y2):
                i[1] = y2
            
            if(i[0]+2 == x1 or i[0]-2 == x1):
                i[0] = x1
            if(i[0]+2 == x2 or i[0]-2 == x2):
                i[0] = x2
            if(i[1]+2 == y1 or i[1]-2 == y1):
                i[1] = y1
            if(i[1]+2 == y2 or i[1]-2 == y2):
                i[1] = y2
                
            if(i[0]+3 == x1 or i[0]-3 == x1):
                i[0] = x1
            if(i[0]+3 == x2 or i[0]-3 == x2):
                i[0] = x2
            if(i[1]+3 == y1 or i[1]-3 == y1):
                i[1] = y1
            if(i[1]+3 == y2 or i[1]-3 == y2):
                i[1] = y2
                
            if(i[0]+4 == x1 or i[0]-4 == x1):
                i[0] = x1
            if(i[0]+4 == x2 or i[0]-4 == x2):
                i[0] = x2
            if(i[1]+4 == y1 or i[1]-4 == y1):
                i[1] = y1
            if(i[1]+4 == y2 or i[1]-4 == y2):
                i[1] = y2
            
            if(i[0]+5 == x1 or i[0]-5 == x1):
                i[0] = x1
            if(i[0]+5 == x2 or i[0]-5 == x2):
                i[0] = x2
            if(i[1]+5 == y1 or i[1]-5 == y1):
                i[1] = y1
            if(i[1]+5 == y2 or i[1]-5 == y2):
                i[1] = y2
        return arr

    def getOriginAndHeightAndWidth(self, arr):
        ...
        p2y = np.min(temp2)
        p4y = np.max(temp2)
        w = self.distance((p1x, p1y), (p2x, p2y))
        h = self.distance((p1x, p1y),(p3x, p3y))
        #print('P1: {0}, {1}'.format(p1x, p1y))
        #print('P2: {0}, {1}'.format(p2x, p2y))
        #print('P3: {0}, {1}'.format(p3x, p3y))
        return (p1x, p1y, h, w)
    

    def getOriginAndHeightAndWidth1(self, arr):
        s = 5
        p1x = np.min(arr[:, 0])
        p2x = np.max(arr[:, 0])
        temp1 = []
        ...
        p2y = np.min(temp2)
        for row in arr:
            if((row[0] == p2x or row[0] <= p2x-s or row[0] <= p2x+s) and row[1] != p2y and not(row[0] == p1x or row[0] <= p1x-s or row[0] <= p1x+s)):
                p3x, p3y = row[0], row[1]
        w = self.distance((p1x, p1y), (p2x, p2y))
        h = self.distance((p2x, p2y),(p3x, p3y))
        # print('P1: {0}, {1}'.format(p1x, p1y))
        # print('P2: {0}, {1}'.format(p2x, p2y))
        # print('P3: {0}, {1}'.format(p3x, p3y))
        return (p1x, p1y, h, w)
    ...
    def crop_cell_hull(self, image, i, text, xscale=1., yscale=1.):
        ...
        if text is not None:
            ...
            txt = self.stage1Tesseract(cv2.imread('/home/vijay/workspace/OpevCV/'+text+'.png'))
            print('{0}: {1}'.format(text, txt))
    
    def crop_table_coord_cell_hulls(self, image, **kwargs):
        for i in range(len(self.cell_hulls)):
            tcoordX, tcoordY = self.cell_table_coord[i]
            text = "{0},{1}".format(tcoordX, tcoordY)
            #print('Cell hull {0}'.format(text))
            self.crop_cell_hull(image, i, text, **kwargs)
    
    def stage1Tesseract(self, img):
        txt = pytesseract.image_to_string(img)
        txt = txt.replace('\r', '').replace('\n', '')
        if txt == '':
            return self.stage2Tesseract(img)
        return txt
    
    def stage2Tesseract(self, img):
